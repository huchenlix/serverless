...
public class MenpiaogoumaiController {
    ...
    @IgnoreAuth
    @GetMapping("/getZhexian")
    public R getZhexian() {
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
        List<MenpiaogoumaiEntity> list = menpiaogoumaiService.selectListCustom();
        List<Date> xDataTemp = list.stream().map(MenpiaogoumaiEntity::getGoumairiqi).distinct().collect(Collectors.toList());


        List<String> xData = new ArrayList<>(7);
        int countTemp = 0;

        for (Date name: xDataTemp) {
            if(countTemp==7){
                break;
            }
            countTemp++;
           String names =  sdf.format(name);
            xData.add(names);
        }
        ...
        xData.forEach(names -> {
            ...
            list.forEach(item -> {
                ...
                if (sdf.format(item.getGoumairiqi()).equals(names)) {
