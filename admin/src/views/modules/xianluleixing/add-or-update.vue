<template>
	<div class="addEdit-block" style="width: 100%;">
		...
		>
			<template >
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="线路类型" prop="xianluleixing">
					<el-input v-model="ruleForm.xianluleixing" placeholder="线路类型" clearable  :readonly="ro.xianluleixing"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else class="input" label="线路类型" prop="xianluleixing">
					<el-input v-model="ruleForm.xianluleixing" placeholder="线路类型" readonly></el-input>
				</el-form-item>
			</template>
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({
				...
				if (data && data.code === 0) {
					 ...
					 this.$http({
					   url: `xianluleixing/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.xianluleixingCrossAddOrUpdateFlag = false;
						 this.parent.search();
